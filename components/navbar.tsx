...
import { IconArrowLeft, IconMoon, IconSun, IconUser } from "@tabler/icons-react";
...
function Navbar({ className }: { className?: string }) {
  ...
  const handleBackButtonClick = () => {
    if (pathname.startsWith("/view/") || pathname.startsWith("/rate/")) {
      router.push("/home");
  ...
  return (
    <div className={cn("fixed top-10 inset-x-0 max-w-2xl mx-auto z-50", className)}>
      <Menu setActive={setActive}>
        ...
        <button
          className="rounded-full w-fit dark:text-white text-black flex items-center justify-center ml-2"
          onClick={toggleTheme}
        >
...
//   const [active, setActive] = useState<string | null>(null);
//   const [isDarkMode, setIsDarkMode] = useState<boolean | undefined>(undefined);  // Initially undefined
//    const [hasPreviousPage, setHasPreviousPage] = useState<boolean>(false);  // New state to track previous page
